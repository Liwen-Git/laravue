            ...
            >
                <template slot-scope="scope">
                    <el-button
                            ...
                            size="mini"
                            @click="handleEdit(scope.$index, scope.row)">{{ $t('edit') }}
                    </el-button>
                    <el-button
    ...
    export default {
        ...
        methods: {
            ...
            handleDelete(index, row) {
                this.$http.delete(`/api/permission-group/${row.id}`).then(() => {
            ...
            handleAddPermissionGroup() {
                this.$refs['addForm'].validate((valid) => {
                    if (valid) {
                        this.$http.post('/api/permission-group', this.addForm).then(() => {
                            addSuccess(this);
                            this.requestData();
                        })
                    } else {
            ...
            handleEditPermissionGroup() {
                this.$refs['editForm'].validate((valid) => {
                    if (valid) {
                        this.$http.patch(`/api/permission-group/${this.nowRowData.row.id}`, this.editForm).then(() => {
                            editSuccess(this)
                        })
                    } else {
                        return false;
