        ...
        <el-card v-for="group in guardNameByPermissions" :key="group.id" style="margin-bottom:20px;">
            <div slot="header">
                <div style="float:right">
                    <el-radio v-model="radio[group.id]" @change="change(group.id)" :label="true">{{ $t('selectAll') }}
                    </el-radio>
                    <el-radio v-model="radio[group.id]" @change="change(group.id)" :label="false">{{ $t('unselectAll')
                        }}
                    </el-radio>
                </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            assignPermission() {
                this.$http.put(`/api/role/${this.$route.params.id}/permissions`, this.rolePermissions).then(response => {
            ...
            loadData() {
                ...
                let permissionGroups = this.$http.get(`/api/guard-name-for-permissions/${this.$route.params.guardName}`);
                let rolePermissions = this.$http.get(`/api/role/${this.$route.params.id}/permissions`);
